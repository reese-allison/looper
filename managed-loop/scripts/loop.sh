...
<!-- Tasks will be generated by planning phase -->
EOF
...
EOF
  ...
  cat > "$LOOP_DIR/PLAN_PROMPT.md" << EOF
...
Tasks should be specific and independently completable.
EOF

  cat > "$LOOP_DIR/PROMPT.md" << EOF
@$LOOP_DIR/goal.md @$LOOP_DIR/plan.md @$LOOP_DIR/activity.md
...
EOF
  ...
  if ! claude -p $SETTINGS_ARG "$(cat "$LOOP_DIR/PLAN_PROMPT.md")"; then
    echo "Error: Planning failed" >&2
    exit 1
  fi

  # Verify plan was created
  if ! grep -q '\- \[ \]' "$LOOP_DIR/plan.md"; then
    echo "Error: No tasks in plan.md" >&2
    exit 1
  fi
